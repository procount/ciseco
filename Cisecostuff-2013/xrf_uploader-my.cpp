...
void execute_command(int fd, const char* command, const char* expected_response, long timeout)
{
    ...
    {
        printf("Invalid response received (RECV:%s, EXPECTED: %s)\n",response,expected_response);
        exit(1);
    }
...
//    execute_command(fd_device, "ATPG", "OK", timeout);
    ...
    if (c_in != '3')
    {
        printf("Invalid response received after ATPG (%c, EXPECT: '3')\n",c_in);
        exit(1);
    }
    ...
    if (c_in != 'W')
    {
        printf("Invalid response received (%c, EXPECT 'W')\n",c_in);
    ...
    while (data != 0)
    {
        ...
        if (c_in != 'R')
        {
            printf("Invalid response received (%c, EXPECT 'R')\n",c_in);
        ...
        if (c_in != 'A')
        {
            printf("Invalid response received (%c, EXPECT 'A')\n",c_in);
    ...
    if (c_in != 'y')
    {
        printf("Invalid response received (%c, EXPECT 'y')\n",c_in);
